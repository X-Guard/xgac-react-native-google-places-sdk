...
func GMSPlaceFieldsFromFields(fields: NSArray) -> [String] {
  var parsedFields: [String] = []
  for field in fields {
    if let nsField = field as? NSString {
      if let parsedField = PLACE_FIELD_MAP[String(nsField)] {
        parsedFields.append(parsedField.rawValue)
      }
    }
  }

  if (parsedFields.count == 0) {
    for field in PLACE_FIELD_MAP.values {
      parsedFields.append(field.rawValue)
    }
  }
